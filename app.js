...
async function saveEntryFromEditor() {
  ...
  const payload = {
    ...
    dateKey: existing?.dateKey || state.editorDateKey || state.currentDate,
  ...
  state.editorDateKey = payload.dateKey;
  state.tempMedia = [];
  await renderAll();
...
async function exportPdfBook() {
  ...
  if (!printWin) return showToast('پنجره PDF باز نشد. پاپ‌آپ را برای اپ مجاز کن.');
  printWin.document.open();
  printWin.document.write(html);
  printWin.document.close();
  showToast('صفحه چاپ باز شد؛ از همان‌جا Save as PDF بگیر.');
}

function buildPrintableBook(entries) {
  const sections = entries.map((entry) => {
    const imageCards = (entry.media || [])
      .filter(m => m.type?.startsWith('image/') && m.dataUrl)
      .map(m => `<figure class="print-photo-card"><img src="${m.dataUrl}" alt="${escapeHtml(m.name || 'image')}" /><figcaption>${escapeHtml(m.name || 'تصویر')}</figcaption></figure>`);

    const paragraphs = String(entry.body || 'متنی ثبت نشده.')
      .split(/\n{1,2}/)
      .map(t => t.trim())
      .filter(Boolean);

    const chunks = [];
    const safeParagraphs = paragraphs.length ? paragraphs : ['متنی ثبت نشده.'];
    let imageCursor = 0;
    safeParagraphs.forEach((paragraph, idx) => {
      chunks.push(`<p>${escapeHtml(paragraph)}</p>`);
      const shouldInject = imageCursor < imageCards.length && (idx === 0 || idx === safeParagraphs.length - 1 || idx % 2 === 1);
      if (shouldInject) {
        const group = imageCards.slice(imageCursor, imageCursor + 2);
        imageCursor += group.length;
        if (group.length) chunks.push(`<div class="print-media-grid">${group.join('')}</div>`);
      }
    });
    if (imageCursor < imageCards.length) {
      chunks.push(`<div class="print-media-grid">${imageCards.slice(imageCursor).join('')}</div>`);
    }

    return `
      <section class="print-entry">
        <header class="print-meta-card">
          <div class="print-date">${escapeHtml(formatFullDate(entry.dateKey))}</div>
          <div class="print-pill">${escapeHtml(MOODS[entry.mood ?? 3].emoji + ' ' + MOODS[entry.mood ?? 3].label)}</div>
          <div class="print-time">${escapeHtml(new Intl.DateTimeFormat(PERSIAN_LOCALE, { timeZone: APP_TIMEZONE, dateStyle: 'medium', timeStyle: 'short' }).format(new Date(entry.updatedAt)))}</div>
        </header>
        <h1>${escapeHtml(entry.title || 'بدون عنوان')}</h1>
        <div class="print-body-flow">${chunks.join('')}</div>
      </section>`;
  }).join('');

  return `<!doctype html><html lang="fa" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Journara PDF</title><style>
    @page{size:A4;margin:16mm}
    *{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact}
    html,body{margin:0;padding:0;background:#fff;color:#152033;font-family:-apple-system,BlinkMacSystemFont,'SF Pro Text','Segoe UI',Tahoma,sans-serif}
    .book{padding:0}
    .print-entry{break-after:page;page-break-after:always;display:block}
    .print-entry:last-child{break-after:auto;page-break-after:auto}
    .print-meta-card{display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin:0 0 10px;padding:10px 12px;border:1px solid #d7e0ea;border-radius:14px;background:linear-gradient(180deg,#f9fbff,#f2f7fd)}
    .print-date{font-weight:700;color:#132238}
    .print-pill,.print-time{font-size:12px;color:#506079;background:#fff;border:1px solid #dbe4ee;border-radius:999px;padding:4px 10px}
    h1{font-size:22px;line-height:1.5;margin:0 0 14px;color:#0f1d34}
    .print-body-flow{font-size:14px;line-height:2;text-align:justify}
    .print-body-flow p{margin:0 0 11px;orphans:3;widows:3}
    .print-media-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10mm;margin:10px 0 14px;break-inside:avoid;page-break-inside:avoid}
    .print-photo-card{margin:0;border:1px solid #d9e4ef;border-radius:16px;padding:5mm;background:#f8fbff;break-inside:avoid;page-break-inside:avoid;box-shadow:0 6px 18px rgba(34,51,84,.08)}
    .print-photo-card img{display:block;width:100%;aspect-ratio:1 / 1.414;object-fit:cover;border-radius:12px;break-inside:avoid;page-break-inside:avoid}
    .print-photo-card figcaption{margin-top:4mm;font-size:11px;line-height:1.6;color:#5c6c84;text-align:center}
    @media print {
      .print-media-grid,.print-photo-card,.print-photo-card img{break-inside:avoid;page-break-inside:avoid}
    }
  </style></head><body><main class="book">${sections}</main><script>window.onload=()=>setTimeout(()=>window.print(),260);</script></body></html>`;
}
...
function toDateKey(d) { return `${d.getUTCFullYear()}-${String(d.getUTCMonth() + 1).padStart(2, '0')}-${String(d.getUTCDate()).padStart(2, '0')}`; }
function parseDateKey(key) { const [y, m, d] = key.split('-').map(Number); return new Date(Date.UTC(y, m - 1, d, 12, 0, 0)); }
