...
async function exportPdfBook() {
  ...
  const html = buildPrintableJournalBook(entries);
  ...
  if (!doc) {
    iframe.remove();
    return showToast('آماده‌سازی خروجی PDF انجام نشد.');
  }
  doc.open();
  doc.write(html);
  doc.close();
  iframe.onload = () => {
    try {
      ...
      showToast('پنجره چاپ باز شد؛ از همان‌جا Save as PDF یا Print to PDF را بزن.');
    } catch (err) {
      console.error(err);
      showToast('باز کردن پنجره چاپ انجام نشد.');
    } finally {
      cleanup();
    }
  };
...
function buildPrintableJournalBook(entries) {
  const pages = entries.map((entry, index) => {
    const mediaHtml = (entry.media || []).filter(m => String(m.type || '').startsWith('image/')).map(media =>
      `<figure class="print-media"><img src="${media.dataUrl || media.url}" alt="${escapeHtml(media.name || 'image')}" /></figure>`
    ).join('');
    const timeLabel = new Date(entry.updatedAt || entry.createdAt || Date.now()).toLocaleTimeString('fa-IR', { hour: '2-digit', minute: '2-digit' });
    return `
      <section class="print-page ${index < entries.length - 1 ? 'page-break' : ''}">
        <header class="print-head">
          <div class="print-brand">Journara</div>
          <div class="print-date">${escapeHtml(formatFullDate(entry.dateKey))}</div>
          <div class="print-meta">${escapeHtml(entry.title || 'بدون عنوان')} • ${escapeHtml(MOODS[entry.mood ?? 3].emoji)} ${escapeHtml(MOODS[entry.mood ?? 3].label)} • ${escapeHtml(timeLabel)}</div>
        </header>
        <article class="print-body">
          <h1>${escapeHtml(entry.title || 'بدون عنوان')}</h1>
          <pre>${escapeHtml(entry.body || 'متنی ثبت نشده.')}</pre>
          ${mediaHtml || '<div class="print-empty">رسانه‌ای برای این صفحه ثبت نشده.</div>'}
        </article>
      </section>`;
  }).join('');
  return `<!doctype html>
  ...
  <head>
    ...
    <title>Journara PDF</title>
    <style>
      @page { size: A4; margin: 14mm; }
      html,body{margin:0;padding:0;background:#fff;color:#111;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",system-ui,sans-serif;-webkit-print-color-adjust:exact;print-color-adjust:exact}
      body{direction:rtl}
      .print-page{min-height:calc(297mm - 28mm); display:flex; flex-direction:column; gap:14px;}
      .page-break{page-break-after:always; break-after:page}
      .print-head{padding:14px 16px;border-radius:18px;background:linear-gradient(135deg,#eff6ff,#dbeafe);border:1px solid #bfdbfe}
      .print-brand{font-size:12px;font-weight:800;color:#1d4ed8;letter-spacing:.04em}
      .print-date{font-size:16px;font-weight:800;color:#0f172a;margin-top:4px}
      .print-meta{font-size:12px;color:#334155;margin-top:8px}
      .print-body{padding:4px 2px 0}
      .print-body h1{font-size:22px;margin:0 0 12px;color:#0f172a}
      .print-body pre{white-space:pre-wrap;word-wrap:break-word;line-height:1.95;font-size:13.5px;margin:0 0 16px;color:#172033}
      .print-media{margin:0 0 14px;break-inside:avoid;page-break-inside:avoid}
      .print-media img{max-width:100%;max-height:210mm;object-fit:contain;border-radius:16px;border:1px solid #cbd5e1;display:block;margin-inline:auto}
      .print-empty{padding:14px 16px;border:1px dashed #cbd5e1;border-radius:14px;color:#64748b;font-size:12px}
    </style>
  </head>
  <body>${pages}</body>
  </html>`;
...
function showToast(text) { const toast = document.createElement('div'); toast.className = 'toast'; toast.textContent = text; document.body.appendChild(toast); setTimeout(() => toast.remove(), 3200); }
