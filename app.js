...
async function exportPdfBook() {
  const ok = await ensureLibrary('pdflib', 'https://unpkg.com/pdf-lib@1.17.1/dist/pdf-lib.min.js');
  if (!ok || !window.PDFLib) return showToast('کتابخانه PDF آماده نشد.');
  const entries = await getAllEntries();
  ...
  const pageWidth = 595, pageHeight = 842, margin = 44;
  ...
  for (const entry of entries) {
    ensureSpace(110);
    page.drawRectangle({ x: margin - 8, y: y - 90, width: pageWidth - (margin * 2) + 16, height: 86, color: rgb(.95, .97, 1) });
    page.drawText((entry.title || 'Untitled').slice(0, 80), { x: margin, y: y - 26, size: 18, font: fontBold, color: rgb(.08, .12, .20) });
    page.drawText(`${formatFullDate(entry.dateKey)} | Mood: ${MOODS[entry.mood ?? 3].emoji} ${MOODS[entry.mood ?? 3].label} | ${new Date(entry.updatedAt).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}`, { x: margin, y: y - 50, size: 10, font, color: rgb(.35, .42, .54) });
    y -= 110;
    for (const line of wrapText((entry.body || '').replace(/\n+/g, ' '), 82)) {
      ensureSpace(18);
      page.drawText(line, { x: margin, y, size: 11, font, color: rgb(.12, .16, .24) });
      y -= 16;
    }
    y -= 8;
    for (const media of entry.media || []) {
      if (!media.type.startsWith('image/') || !media.dataUrl) continue;
      ensureSpace(220);
      try {
        const bytes = dataUrlToUint8Array(media.dataUrl);
        const img = media.type.includes('png') ? await pdfDoc.embedPng(bytes) : await pdfDoc.embedJpg(bytes);
        const dims = fitBox(img.width, img.height, pageWidth - margin * 2, 210);
        page.drawImage(img, { x: margin, y: y - dims.height, width: dims.width, height: dims.height });
        y -= dims.height + 14;
      } catch (err) { console.warn('image skipped', err); }
    }
    y -= 22;
  }
  const bytes = await pdfDoc.save();
  await saveFileSmart(new Blob([bytes], { type: 'application/pdf' }), `journara-book-${state.currentDate}.pdf`, 'application/pdf');
}
